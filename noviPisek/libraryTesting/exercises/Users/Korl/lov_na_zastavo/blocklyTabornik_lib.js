var getContext = function(display, infos, curLevel) {
    var localLanguageStrings = {
        ...
        sl: {
            label: {
                ...
                jump: "skoči čez oviro",
                jumpLeft: "skoči čez oviro levo",
                jumpRight: "skoči čez oviro desno",
                down: "pojdi dol",
                ...
                onHole: "na gnezdu",
                onObservableItem: "na predmetu X",
                itemObserve: "preuči predmet X",
            ...
            code: {
                ...
                onHole: "naLuknji",
                onObservableItem: "naPredmetuX",
                itemObserve: "preučiPredmetX",
            ...
            messages: {
                ...
                alreadyTransporting: "Tabornik že nosi predmet.",
                notTransporting: "Tabornik skuša spustiti predmet, vendar ga ne nosi.",
                successDroppedAllObjects: "Čestitamo, spustil(-a) si vse predmete!",
                successMarkersPainted: "Čestitamo, Tabornik je pravilno pobarval polja!",
                failureMarkersPainted: "Tabornik ni pobarval pravega polja.",
                successPickedAllCollectibles: "Super! Tabornik je preizkušnjo uspešno opravil!",
                failurePickedAllCollectibles: "Ojoj! Tabornik ni prehodil vseh kamnov.",
                successReachGeenArea: "Super! Taborniku je uspelo priti na cilj.",
                failureReachGeenArea: "Ojoj! Taborniku ni uspelo priti na cilj. Poskusi še enkrat.",
                successOneMarbleInHole: "Čestitamo, frnikola je v luknji!",
                successAllMarblesInHoles: "Čestitamo, vse frnikole so v luknjah!",
                failureOneMarbleInHole: "Frnikola ni v luknji!",
                failureAllMarblesInHoles: "Vse frnikole niso v luknjah!",
                leavesGrid: "Ojoj! Tabornik je zašel s poti.",
                successObservedAllItems: "Robot je preučil vse predmete X in dosegel zeleno polje!",
                failureObservedAllItems: "Robot ni preučil vseh predmetov X!"

            },
            description: {
                platformAbove: "ploščadZgoraj(): Ali obstaja ploščad nad robotkom?",
                obstacleInFront: "oviraSpredaj(): Ali obstaja ovira pred robotkom?",
                onHole: "naLuknji(): Ali je Tabornik na luknji?",
                onTransportable: "naPredmetu(): Ali je Tabornik na predmetu, ki ga lahko pobere?",
                onRoundObject: "naKrogu(): Ali je Tabornik na okroglem predmetu?",
                onSquareObject: "naKvadratu(): Ali je Tabornik na kvadratnem predmetu?",
                onTriangleObject: "naTrikotniku(): Ali je Tabornik na trikotnem predmetu?",
                onDottedObject: "naPikčastemPredmetu(): Ali je Tabornik na pikčastem predmetu?",
                onFullObject: "naPolnemPredmetu(): Ali je Tabornik na polnem predmetu?",
                onStripedObject: "naČrtastemPredmetu(): Ali je Tabornik na črtastem predmetu?",
                paintOnCell: "pobarvanoPolje(): Ali je polje, na katerem je Tabornik, pobarvano?",
                gridEdgeInFront: "robMrežeSpredaj(): Ali je rob mreže pred robotkom?",
                gridEdgeEast: "robMrežeDesno(): Ali je rob mreže desno od robotka?",
                gridEdgeWest: "robMrežeLevo(): Ali je rob mreže levo od robotka?",
                gridEdgeAbove: "robMrežeZgoraj(): Ali je rob mreže nad robotkom?",
                gridEdgeBelow: "robMrežeSpodaj(): Ali je rob mreže pod robotkom?",
                platformInFront: "ploščadSpredaj(): Ali obstaja ploščad pred robotkom?",
                numberUnder: "številkaPolja(): Številka polja, na katerem je Tabornik.",
                col: "stolpec(): Stolpec na katerem je Tabornik",
                row: "vrstica(): Vrstica na kateri je Tabornik",
                paintNorth: "pobarvanaZgoraj(): Ali je pobarvano polje nad poljem, na katerem je Tabornik?",
                paintNorthWest: "pobarvanaZgorajLevo(): Ali je pobarvano polje levo nad poljem, na katerem je Tabornik?",
                paintNorthEast: "pobarvanaZgorajDesno(): Ali je pobarvano polje desno nad poljem, na katerem je Tabornik?",
                writeNumber: "zapišiŠtevilko(številka): Zapiše številko v polje, na katerem je Tabornik."
            },
            obstacle: "Tabornik se je zaletel v oviro!",
            startingBlockName: "Program",
            exit_grid: "Tabornik je zapustil mrežo!",
            nothingToPickUp: "Ni predmetov, ki bi jih lahko pobral.",
            alreadyTransportingObject: "Tabornik že nosi predmet.",
            jumpOutsideGrid: "Tabornik skuša skočiti izven mreže!",
            jumpObstacleBlocking: "Tabornik skuša skočiti, vendar zaradi ovire ne more!",
            jumpNoPlatform: "Tabornik skuša skočiti, vendar ni nobene ploščadi!",
            downOutsideGrid: "Tabornik se skuša premakniti izven mreže!",
            downNoPlatform: "Tabornik se skuša premakniti dol, vendar spodaj ni ploščadi!",
            falls: "Tabornik pada v prazno.",
            willFallAndCrash: "Tabornik bo padel z ploščadi in se poškodoval!",
            dropWithoutItem: "Tabornik skuša spustiti predmet, vendar ga ne nosi.",
            ...
            noObservableItem: "Na polju ni predmeta X.",
            itemObserved: "Tabornik je že preučil predmet X.",
            turnToItemString: "Obrni tabornika v smer: %1"
        },
        none: {
            comment: {}
        }
    ...
    // Custom defined functions for tasks at Pisek

    context.robot.jumpLeft = function (callback) {
        if (!infos.hasGravity) {
            throw("Error: can't jump without gravity");
        }
        if (context.lost) {
        ...
        var item = context.getRobotItem(context.curRobot);
        if (isOutsideGrid(item.row, item.col - 2)) {
            context.lost = true;
            throw(strings.jumpOutsideGrid);
        }
        var obstacle = context.getItems(item.row, item.col - 2, {category: "obstacle"});
        if (obstacle.length > 0) {
            context.lost = true;
            throw(strings.jumpObstacleBlocking);
        }
        var platforms = context.getItems(item.row, item.col - 1, {category: "platform"});
        if (platforms.length == 0) {
            context.lost = true;
            throw(strings.jumpNoPlatform);
        }
        context.nbMoves++;
        moveRobot(item.row, item.col - 2, item.dir, callback);
    };

    context.robot.jumpRight = function (callback) {
        if (!infos.hasGravity) {
            throw("Error: can't jump without gravity");
        }
        if (context.lost) {
    ...
    context.robot.itemObserve = function (callback) {
        var robot = context.getRobotItem(context.curRobot);
        var items = context.getItems(robot.row, robot.col, {category: "observable"});
        if (items.length == 0) {
            context.lost = true;
            throw(strings.noObservableItem);
        }
        for (var iItem = 0; iItem < items.length; iItem++) {
            var curItem = items[iItem];
            if (curItem.observed) {
                context.lost = true;
                throw(strings.itemObserved);
            }
            curItem.observed = true;
            context.nbObservedItems++;
        }

        context.callCallback(callback);
    };

    context.robot.onObservableItem = function (callback) {
        var robot = context.getRobotItem(context.curRobot);
        var items = context.getItems(robot.row, robot.col, {category: "observable"});
        var result = false;
        if (items.length > 0) {
            for (var iItem = 0; iItem < items.length; iItem++) {
                if (!items[iItem].observed) {
                    result = true;
                }
            }
        }
    ...
    //context.robot.turnToItem = function(id, callback) {
...
//      context.blocklyHelper.updateSize();
      ...
      robot: {
         actions: [
            ...
            /*
             { name: "turnToItem",
                blocklyJson: {
               "message0": strings.turnToItemString,
               "args0": [
                   {"type": "field_dropdown",
                    "name": "PARAM_0",
                    "options":
                        [
                            ["zeleni kvadrat", "green"],
                            ["predmet X", "observable"]
                        ]
                     }
                   ]
               }
         ...
         sensors: [
			...
			{ name: "onObservableItem",   yieldsValue: true }
         ]
      },
      transport: {
         sensors: [
   ...
   var redisplayItem = function(item) {
      ...
      /*
      if (itemType.img2) {
          item.element = paper.image(itemType.img2, x, y, item.side * item.nbStates * scale, item.side * scale);
      } else
      TODO: After observing item redisplay item
       */
      if (itemType.img) {
...
var robotEndConditions = {
   ...
   // checkEndCondition custom for Pisek
   // Condition tests if all items were observed

   checkObservedAllItems : function(context, lastTurn) {
      var solved = true;
      var nbObservables = 0;
      var robot = context.getRobotItem(context.curRobot);
      ...
      for (var iRow = 0; iRow < context.tiles.length; iRow++) {
         var row = context.tiles[iRow];
         for (var iCol = 0; iCol < row.length; iCol++) {
            var items = context.getItems(iRow, iCol, {category: "observable"});
            nbObservables += items.length;
         }
      }
      if (nbObservables == context.nbObservedItems && paints.length != 0) {
         context.success = true;
         throw(window.languageStrings.messages.successObservedAllItems);
      }
      if (nbObservables == context.nbObservedItems) {
         context.success = false;
         throw(window.languageStrings.messages.failureReachGeenArea);
      }
      if (lastTurn) {
         context.success = false;
         throw(window.languageStrings.messages.failureObservedAllItems);
      }
   }
};
