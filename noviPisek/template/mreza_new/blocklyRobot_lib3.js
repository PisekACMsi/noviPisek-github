...
var localLanguageStrings = {
   sl: {
      ...
      label: {									//Imena ukazov
         move: "premakni se za %1 v smer %2",
...
// recursive setTimeout loop function
function loopFunction(i, func){
   if(i > 0){
      func();
      setTimeout(loopFunction(i-1, func), 1000);
   }
}
...
var getContext = function(display, infos) {
   ...
   // backend functions
	context.getRobotItem = function(robotRank) {
		var items = this.getItems(undefined, undefined, {isRobot: true, rank: robotRank});
		if(items.length == 0)  items = this.getItems(undefined, undefined, {isRobot: true});
		return items[0];
	};
   ...
   context.triggerItem = function(item) {
      var itemType = infos.itemTypes[item.type];
      for (var key in itemType) {
         if( itemType[key] instanceof Array ){
            var index = (itemType[key].indexOf(item[key])+1) % itemType[key].length;
            item[key] = itemType[key][index];
         }
      }  
   };
   var moveRobot = function(newRow, newCol, newDir, callback) {
      ...
      console.log(robot, robot.element);
		...
		// coins
		var coins = context.getItems(newRow, newCol, {isCoin: true});
		// buttons
      var buttons = context.getItems(newRow, newCol, {isButton: true});
      var ids = [];
      ...
      // Execute animations
		if (context.display) {
            ...
            // console.log(coins, context.nbCoins);
			...
			if (changeLoc) {
				attr = context.itemAttributes(robot);
				context.raphaelFactory.animate("animRobot" + iRobot + "_" + Math.random(), robot.element, attr, infos.actionDelay);
				context.delayFactory.createTimeout("triggerItems" + iRobot + "_" + Math.random(), function() {
               for(var ii = 0; ii < ids.length; ii++) {
                  var bConnected = context.getItems(undefined, undefined, {id: ids[ii]});
                  for(var jj = 0; jj < bConnected.length; jj++) {
                     bc = bConnected[jj];
                     context.triggerItem(bc);
                     context.redisplayItem(bc);
                  }
               }
            }, infos.actionDelay);
			}
				...
				if (infos.actionDelay > 0) {
					context.delayFactory.createTimeout("rotateRobot" + iRobot + "_" + Math.random(), function() {
						robot.element.attr(attr);
					}, infos.actionDelay / 2);
				}
				else {
   ...
   context.itemAttributes = function(item) {
      ...
      if (item.dir != undefined) {
         ...
         x = x - (dirToState[item.dir] * item.side * this.props.scale);
      }
      ...
      return { x: x, y: y, width: item.side * item.nbStates * this.props.scale, height: item.side * this.props.scale, "clip-rect": clipRect};
   };
   ...
   context.fall = function(item, coords, callback) {
      var row = coords.row;
      var platforms = this.getItems(row+1, coords.col, {isPlatform: true});
      while ((!this.isOutsideGrid(row + 1, coords.col)) && (platforms.length == 0)) {
         row++;
         platforms = this.getItems(row+1, coords.col, {isPlatform: true});
      }
      if (this.isOutsideGrid(row + 1, coords.col)) {
         this.lost = true;
   ...
   context.checkTileAllowed = function(row, col) {
      if (this.isOutsideGrid(row, col) || (this.tiles[row][col] == 0)) {
         if (infos.ignoreInvalidMoves) {
            return false;
         }
         throw(this.strings.messages.leavesGrid);
      }
      var itemsInFront = this.getItems(row, col, {isObstacle: true});
   ...
   function createItem(newItem) {
      ...
      if (context.isOutsideGrid(newItem.row, newItem.col)) {
      ...
      if ((infos.actionDelay > 0) && (context.display)) {
         context.delayFactory.createTimeout("addItem" + context.robotRankInUse + "_" + Math.random(), function() {
            addItem();
         }, infos.actionDelay / 2);
      } else {
         addItem();
   ...
   function gridEdgeCoord(row, col, callback) {
      var gridEdge = false;
      if (context.isOutsideGrid(row, col)) {
         gridEdge = true;
   ...
   function addPlatform(row, col, callback) {
      var platforms = context.getItems(row, col, {isPlatform: true});
   ...
   function paint(row, col, paintType, callback) {
      ...
      if ((paintItems.length != 0) && (paintItems[0].type != paintType)) {
         var listItems = context.getItems(row, col, {category: "paint"});
         context.destroyItems(listItems);
   ...
   var getCoordsInDir = function(robot, absDir, amount) {
      return {
         row: robot.row + amount*context.props.delta[absDir][0],
         col: robot.col + amount*context.props.delta[absDir][1]
      };
   };
   context.robot.move = function(amount, dir, callback){
      if (context.lost) {
			return;
		}
      var robot = context.getRobotItem(context.robotRankInUse);
      ...
      var newDir = context.props.dirNames.indexOf(dir);
		var coords = getCoordsInDir(robot, newDir, amount);
		if (!context.checkTileAllowed(coords.row, coords.col)) {
			context.waitDelay(callback);
		}
		if (infos.hasGravity) {
			context.fall(robot, coords, callback);
		}
		else {
			context.nbMoves++;
			moveRobot(coords.row, coords.col, newDir, callback);
		}
   }
   ...
   context.robot.jump = function(callback) {
      ...
      var item = context.getRobotItem(context.robotRankInUse);
      if (context.isOutsideGrid(item.row - 2, item.col)) {
         context.lost = true;
      ...
      var platforms = context.getItems(item.row - 1, item.col, {isPlatform: true});
      if (platforms.length == 0) {
         context.lost = true;
   ...
   context.robot.down = function(callback) {
      ...
      var platforms = context.getItems(item.row + 3, item.col, {isPlatform: true});
      if (platforms.length == 0) {
        context.lost = true;
   ...
   context.robot.platformInFront = function(callback) {
      ...
      var platforms = context.getItems(robot.row + 1, col, {isPlatform: true});
   ...
   context.robot.platformAbove = function(callback) {
      var robot = context.getRobotItem(context.robotRankInUse);
      var platforms = context.getItems(robot.row - 1, robot.col, {isPlatform: true});
      context.runner.noDelay(callback, (platforms.length > 0));
   }
   ...
   context.robot.east = function(callback) {
      ...
      var item = context.getRobotItem(context.robotRankInUse);
      if (!context.checkTileAllowed(item.row, item.col + 1)) {
         context.waitDelay(callback);
      } else {
         context.nbMoves++;
   ...
   context.robot.north = function(callback) {
      ...
      var item = context.getRobotItem(context.robotRankInUse);
      if (!context.checkTileAllowed(item.row - 1, item.col)) {
         context.waitDelay(callback);
      } else {
         context.nbMoves++;
   ...
   context.robot.trans_row = function(id, callback) {
      var transportable = findTransportable(id);
      var res = 0;
      if (transportable != null) {
   ...
   context.robot.transportableShape = function(callback) {
	...
	context.robot.pickTransportable = function(callback) {
		...
		for(i=0; i<transportables.length; i++){
			...
			if( (!itemType.isRobot) || itemType.rank != context.robotRankInUse) break;
		}
      ...
      if(infos.itemTypes[transportable.type].countTransport) context.fullTransport = true;
		context.transportingItems.push( transportable );
		context.waitDelay(function() {
			if (context.display) {
				transportable.element.remove();
				// if(context.redisplayRobotTransporting(robot, transportable)) context.redisplayItem(robot); //MULTIROBOT optional
			}
			callback();
		});
	};
	context.robot.dropTransportable = function(callback) {
		var robot = context.getRobotItem(context.robotRankInUse);
		if (context.transportingItems.length == 0) {
			throw(context.strings.messages.notTransporting);
		}
      item = context.transportingItems.pop()

      if(infos.itemTypes[item.type].countTransport) context.fullTransport = false;
		...
		context.waitDelay(function() {
			...
			if (context.display) {
				...
				// if(context.redisplayRobotTransporting(robot, item)) context.redisplayItem(robot); //MULTIROBOT optional
			}
			callback();
		});
	};
   ...
   context.customBlocks = {
      robot: {
         actions: [
            ...
            // mode robot amount dir
            
            { 
               name: "move", 
               category: "actions",
               handler: context.robot.move,
               blocklyJson: {
                  "args0": [
                     {
                        "type": "field_number", 
         ...
         sensors: [
            // sensor if it has property on grid
            // sensor to get property value on grid
            // sensor if trans has property
            // sensor to get trans property value
            ...
            // bool if on cell
            { name: "oniItemType",    yieldsValue: true },
            { name: "onProperty",      yieldsValue: true },
            // bool if item in direction
            { name: "itemInDirRel",    yieldsValue: true },
            { name: "itemInDirAbs",    yieldsValue: true },
            // get Transportable property
            { name: "transportableProperty", yieldsValue: true },
            // get property on cell
            { name: "propertyUnder",         yieldsValue: true },
            ...
            { name: "trans_number", yieldsValue: true,                     },
            { name: "trans_exists", yieldsValue: true,   params: [null],   },
            { name: "trans_row",    yieldsValue: true,   params: [null],   },
            { name: "trans_col",    yieldsValue: true,   params: [null],   },
         ]
      },
   };
...
var robotEndConditions = {
   ...
   // check Items on Items
	...
	checkPickedAllCoins: function(context, lastTurn) {
		var solved = true;
		for (var iRow = 0; iRow < context.tiles.length; iRow++) {
			var row = context.tiles[iRow];
			for (var iCol = 0; iCol < row.length; iCol++) {
				var coins = context.getItems(iRow, iCol, {isCoin: true});
				if (coins.length != 0) {
					solved = false;
				}
			}
		...
		if (solved) {
			context.success = true;
			throw(window.languageStrings.messages.successPickedAllCoins);
		}
		if (lastTurn) {
			context.success = false;
			throw(window.languageStrings.messages.failurePickedAllCoins);
		}
	},
