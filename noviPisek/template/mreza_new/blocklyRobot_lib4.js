...
var getContext = function(display, infos) {
   ...
   context.fall = function(item, coords, callback) {
      var row = coords.row;
      var platforms = this.getItems(row+1, coords.col, {category: "platform"});
      while ((!isOutsideGrid(row + 1, coords.col)) && (platforms.length == 0)) {
         row++;
         platforms = this.getItems(row+1, coords.col, {category: "platform"});
      }
   ...
   var moveRobot = function(newRow, newCol, newDir, callback) {
      ...
      // If the robot turns and moves at the sime time, we do an instant turn (for now).
		...
		while (coins.length > 0) {
			...
			context.items.splice(coin.index, 1);
			coins.splice(0, 1);
			context.nbCoins++;
		}
		
		//custom button + switch COMBO
		function button(row, col){
			var buttons = context.getItems(row, col, {isButton: true});
			if(buttons.length > 0){
				var button = buttons[0];
				var doors = context.getItems(undefined, undefined, {id: infos.itemTypes[button.type].id});
